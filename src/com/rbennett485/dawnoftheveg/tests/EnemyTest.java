...
public class EnemyTest {
	...
	public void testClone() {
		...
		assertEquals(orange.bounds.lowerLeft.x, orangeClone.bounds.lowerLeft.x,
				0.0001);
		assertEquals(orange.bounds.lowerLeft.y, orangeClone.bounds.lowerLeft.y,
				0.0001);
	}
	...
	public void testHitHelper(Enemy enemy) {
		int hp = enemy.hp;
		Projectile projA = new ProjectileA(new Vector2(0, 0),
				new Vector2(1, 1), (int) TowerA.TOWER_A_DAMAGE,
				TowerA.TOWER_A_RANGE);
		enemy.hit(projA);
		assertEquals(enemy.hp, hp - (int) TowerA.TOWER_A_DAMAGE < 0 ? 0 : hp
				- (int) TowerA.TOWER_A_DAMAGE);
		enemy.hp = 4;
		enemy.hit(projA);
